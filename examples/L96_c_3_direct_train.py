...
data = np.loadtxt('../data/L96_c_3_direct.dat')
input_training = torch.from_numpy(data[::10,0].reshape(-1,1))
input_training = input_training.float()
output_training = torch.from_numpy(data[::10,1].reshape(-1,1))
...
torch.save(net, 'L96_c_3_direct_trained.pt')
...
plt.tight_layout()
plt.savefig('L96_c_3_direct_trained.pdf')
plt.close()
